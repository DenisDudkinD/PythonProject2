import pytest
from src.dto.actor import ActorCreate
from src.main import create_actor
...
def test_add_actor_negative():
    actor = 23423
    repo = MockActorRepo()
    svc = actor_service.ActorService(repo)
    with pytest.raises(TypeError) as e:
        svc.add_actor(actor)
    assert str(e.value) == "Expected actor, got something else"


def test_update_actor_id_negative():
    Actor = 232
    repo = MockActorRepo()
    svc = actor_service.ActorService(repo)
    with pytest.raises(TypeError) as e:
